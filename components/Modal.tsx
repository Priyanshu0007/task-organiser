...
function Modal() {
  ...
  return (
    ...
    <Transition appear show={isOpen} as={Fragment}>
      <Dialog as='form' className="relative z-10 " onSubmit={e=>handleSubmit(e)} onClose={closeModal}>
       <Transition.Child
          as={Fragment}
        ...
        <Transition.Child
          ...
          leaveTo="opacity-0"
        >
          <Dialog.Panel className='w-full max-w-md tranform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shaow-xl transition-all'>
            <Dialog.Title as='h3' className="text-lg font-medium leading-6 text-gray-900 pb-2">
                Add a Task
            </Dialog.Title>
            <div className='mt-2'>
                <input type='text' value={newTaskInput} onChange={(e)=>setNewTaskInput(e.target.value)} placeholder='Enter a task here...' className='w-full border border-gray-300 rounded-md outline-none p-5' />
            </div>
            <TaskTypeRadioGroup/>
            <div>
                <button type='button' onClick={()=>imagePickerRef.current?.click()} className='w-full border boder-gray-300 rounded-md outlline-none p-4 focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2'>
